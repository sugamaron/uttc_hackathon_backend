...
func InsertItemDao(item model.ItemForRegistration) error {
	const sql_insert = "INSERT INTO item(item_id, title, category_id, lesson_id, registrant, registration_date, updater, update_date, description, url, likes) VALUE(?,?,?,?,?,?,?,?,?,?,?)"
	_, err := db.Exec(sql_insert, item.ItemId, item.Title, item.CategoryId, item.LessonId,
		item.Registrant, item.RegistrationDate, item.Updater, item.UpdateDate, item.Description, item.Url, item.Likes)
	if err != nil {
...
//func GetLikedItemsDao([]model.LikedItemId) (*sql.Rows, error) {
//	const sql_get = "SELECT item_id, title, registrant, registration_date, update_date, likes FROM item WHERE item_id = ?"
//	rows, err := db.Query(sql_get, itemId)
//	if err != nil {
//		log.Printf("fail: db.Query, %v\n", err)
//		return nil, err
...
func GetLikedItemsDao(userId string) (*sql.Rows, error) {
	const sql_get = "SELECT item_id, title, registrant, registration_date, update_date, likes FROM item INNER JOIN likes ON item.item_id = likes.item_id WHERE likes.user_id = ?"
